...
const FloatingMenu = () => {
  ...
  const handleHome = () => {
    if (navigateType === 'PUSH') {
      navigate(-1); // Return to the previously monitored session on the stream health dashboard
    } else {
      navigate('/health'); // Navigate to the stream health dashboard and start monitoring the latest session
    }
